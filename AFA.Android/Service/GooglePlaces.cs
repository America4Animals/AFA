...
namespace AFA.Android.GooglePlacesApi
{
    public class GooglePlaces
    {
        ...
        // Google API Key
		//private const string ApiKey = "[ENTER GOOGLE API KEY HERE]"; // place your API key here

        private PlacesList _placesList;
        private bool _reportedGooglePlacesRetrieved = false;
        // GooglePlaceId, Id
        private Dictionary<string,string> _reportedGooglePlaces; 
        ...
        /// <param name="callback"></param>
        public async void Search(double latitude, double longitude, string types, Action<PlacesList> callback)
        {
            var client = new WebClient();
            InitSearch();

			//string url = string.Format(PlacesSearchUrl, latitude, longitude, ApiKey, types);
			string url = string.Format(PlacesSearchUrl, latitude, longitude, _apiKey, types);
            Log.Info("PlacesUrl", url);

            var placeList = await client.DownloadStringTaskAsync(new Uri(url));
            //_placesList = placeList.FromJson<PlacesList>();
            _placesList = JsonConvert.DeserializeObject<PlacesList>(placeList);
        ...
        /// <param name="callback"></param>
        public async void Search(double latitude, double longitude, string types, string name, Action<PlacesList> callback)
        {
        ...
        /// <param name="longitude"></param>
        /// <param name="callback"></param>
        public async void Search(string nextPageToken, double latitude, double longitude, Action<PlacesList> callback)
        {
            var client = new WebClient();
            InitSearch();
			//string url = string.Format(MorePlacesSearchUrl, latitude, longitude, ApiKey, nextPageToken);
			string url = string.Format(MorePlacesSearchUrl, latitude, longitude, _apiKey, nextPageToken);
            Log.Info("PlacesUrl", url);

            var placeList = await client.DownloadStringTaskAsync(new Uri(url));
            //callback(placeList.FromJson<PlacesList>());
            callback(JsonConvert.DeserializeObject<PlacesList>(placeList));
        ...
        public void GetDetails(string reference, Action<PlaceDetails> callback)
        {
			...
			string url = string.Format(PlaceDetailsUrl, _apiKey, reference);
            ...
            client.DownloadStringCompleted += (sender, args) =>
                                                  {
                                                      ...
                                                      callback(JsonConvert.DeserializeObject<PlaceDetails>(placeDetails));
                                                  };
        ...
        //{
        //    var crueltySpotsService = new CrueltySpotsService();
        //    crueltySpotsService.GetAllGooglePlacesAsync(r =>
        //                                                   {
        ...
        //                                                       {
        //                                                           if (
        //                                                               !_reportedGooglePlaces.ContainsKey(
